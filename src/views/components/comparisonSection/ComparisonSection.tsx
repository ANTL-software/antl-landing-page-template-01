...
function ComparisonStatusIcon({ value }: { value: ComparisonCell }) {
  const StatusIcon = value === "included" ? FiCheck : FiMinus;
  ...
  return <section className="comparison-section section">
    <div className="comparison-section__content shell">
      ...
      <div className="comparison-section__cards" aria-label="Comparatif des fonctionnalités">
        {comparison.columns.map((column, columnIndex) => <article className="comparison-section__card" key={column}><h3>{column}</h3><ul>{comparison.rows.map((row) => <li key={row.label}><span aria-label={row.values[columnIndex] === "included" ? "Inclus" : "Non inclus"}><ComparisonStatusIcon value={row.values[columnIndex]} /></span>{row.label}</li>)}</ul></article>)}
      </div>
    </div>
  </section>;
}
